...
urlpatterns = [
    ...
    # url(r'^collections/(?P<pk>\w+)/groups/(?P<group_id>\w+)/items/$', views.CollectionItemList.as_view(), name='collection-items'),
    # url(r'^collections/(?P<pk>\w+)/groups/(?P<group_id>\w+)/items/(?P<item_id>\w+)/$', views.ItemDetail.as_view(), name='collection-item-detail')
]
